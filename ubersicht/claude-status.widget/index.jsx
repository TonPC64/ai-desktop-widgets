...
// clicking the PLAN QUOTA header toggles it. Dragging the header moves
// the whole card. Both need "Interaction" enabled for this widget from
// Übersicht's menu-bar icon (▸ next to the widget name), plus granting
// Übersicht Accessibility access in System Settings when macOS prompts —
// without that, mouse events never reach the widget at all.
...
const POS_KEY = "claude-status-pos-v2";
const BASE_LEFT = 24;
const BASE_TOP = 36;
const GRID = 12;
...
// Übersicht already only renders its widget layer on the main display (its
// docs describe widget positioning as relative to "the screen", singular),
// so no extra screen-detection is needed here.
export const className = `
  ...
  color: ${INK};
  background:
    linear-gradient(135deg, rgba(255,255,255,0.13) 0%, rgba(255,255,255,0.04) 60%, rgba(120,180,255,0.06) 100%),
    rgba(18, 18, 24, 0.55);
  ...
  box-shadow:
    ...
    0 0 60px rgba(100,160,255,0.06);
  z-index: 1;
`;
...
export const render = ({ output, chartOpen }, dispatch) => {
  ...
  return (
    <div ref={restorePos}>
      ...
      {/* footer: 7-day + this-month window */}
      {(d.week || d.month) && (
        <div
          style={{
            marginTop: 10,
        ...
        >
          ...
          {d.month && (
            <div style={{ marginTop: 2 }}>
              ...
              {fmtCost(d.month.cost)} · {fmtTok(d.month.tokens)} tokens
            </div>
          )}
        </div>
      )}
